...
class Frustum extends AbstractShape {
    ...
    override public function updateFromMatrix(matrix:Mat4) {
        var data:Array<Float> = getRawData(Mat4.transpose(matrix ,new Mat4()));

        _planes[PlanePosition.LEFT] = Vec4.normalize(new Vec4(
        data[12] + data[0],
        data[13] + data[1],
        data[14] + data[2],
        data[15] + data[3]
        ) ,new Vec4());
        _planes[PlanePosition.RIGHT] =  Vec4.normalize(new Vec4(
        data[12] - data[0],
        data[13] - data[1],
        data[14] - data[2],
        data[15] - data[3]
        ) ,new Vec4());
        _planes[PlanePosition.BOTTOM] =  Vec4.normalize(new Vec4(
        data[12] + data[4],
        data[13] + data[5],
        data[14] + data[6],
        data[15] + data[7]
        ) ,new Vec4());
        _planes[PlanePosition.TOP] =  Vec4.normalize(new Vec4(
        data[12] - data[4],
        data[13] - data[5],
        data[14] - data[6],
        data[15] - data[7]
        ) ,new Vec4());
        _planes[PlanePosition.NEAR] =  Vec4.normalize(new Vec4(
        data[8],
        data[9],
        data[10],
        data[11]
        ) ,new Vec4());
        _planes[PlanePosition.FAR] =  Vec4.normalize(new Vec4(
        data[12] - data[8],
        data[13] - data[9],
        data[14] - data[10],
        data[15] - data[11]
