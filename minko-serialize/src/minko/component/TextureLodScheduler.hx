...
import minko.component.AbstractLodScheduler;
import glm.Mat4;
import glm.Vec2;
import glm.Vec3;
import glm.Vec4;
...
class TextureLodScheduler extends AbstractLodScheduler {
    ...
    override public function rendererSet(renderer:Renderer):Void {
    ...
    override public function lodInfo(resource:ResourceInfo, time:Float) {
        var lodInfo:AbstractLodScheduler.LodInfo = new AbstractLodScheduler.LodInfo();
	...
	*/
    ...
    public function computeRequiredLod(resource:TextureResourceInfo, surface:Surface) {
        return  if( masterLodScheduler.streamingOptions.streamedTextureLodFunction ){
            masterLodScheduler.streamingOptions.streamedTextureLodFunction(Math.POSITIVE_INFINITY, resource.maxLod, resource.maxLod, 0.0, surface)
        }else{
