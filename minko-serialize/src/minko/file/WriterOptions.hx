package minko.file;
import minko.file.Options.TextureFunction;
import minko.file.Options.MaterialFunction;
import minko.file.Options.UriFunction;
import Array;
import glm.Vec2;
...
class WriterOptions {
    ...
    public function textureMaxSize(textureType):Vec2 {
    ...
    public function textureScale(textureType):Vec2 {
        return textureOptions(textureType).textureScale;
    }

    public function textureScaleValue(textureType, value:Vec2) {
        if (IsNullOrEmpty(textureType)) {
            for (textureOption in _textureOptions) {
