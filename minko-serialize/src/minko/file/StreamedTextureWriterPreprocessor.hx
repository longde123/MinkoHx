...
class StreamedTextureWriterPreprocessorOptions {
    ...
    public function copyFrom(f:StreamedTextureWriterPreprocessorOptions):Void {
        this.flags = f.flags;
    }
}

class StreamedTextureWriterPreprocessor extends AbstractWriterPreprocessor {

    private var _options:StreamedTextureWriterPreprocessorOptions;
    ...
    public var options(null, set):StreamedTextureWriterPreprocessorOptions;
    ...
    override public function process(_node:Dynamic, assetLibrary:AssetLibrary) {
        ...
        if ((_options.flags & StreamedTextureWriterPreprocessorOptions.computeVertexColor) != 0) {
        ...
        for (surfaceNode in surfaceNodes.nodes()) {
            ...
            for (surface in surfaces) {
                ...
                if (geometry.hasVertexAttribute("uv") && material.data.hasProperty("diffuseMap")) {
                    computeVertexColorAttributes2(geometry, material, assetLibrary);
                }
            }
        }
    }

    public function computeVertexColorAttributes2(geometry:Geometry, material:Material, assetLibrary:AssetLibrary) {
...
//defaultColorVertexBufferData.resize(numVertices * colorVertexBufferVertexSize);
            defaultColorVertexBufferData = [for (i in 0 ...numVertices * colorVertexBufferVertexSize) 0];
    ...
    public function sampleColor(width, height, numComponents, textureData:Bytes, uv:Vec2, color:Vec4) {
        if ((_options.flags & StreamedTextureWriterPreprocessorOptions.smoothVertexColor) != 0) {
