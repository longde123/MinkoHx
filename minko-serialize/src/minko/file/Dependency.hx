...
//serializeGeometry(dependency:Dependency, assetLibrary:AssetLibrary, geometry:Geometry, resourceId:Int, options:Options, writerOptions:WriterOptions, userDefinedDependency:Array<SerializedAsset>)
typedef GeometryWriterFunction = Dependency -> AssetLibrary -> Geometry -> Int -> Options -> WriterOptions -> Array<SerializedAsset> -> SerializedAsset;
//serializeTexture(dependency:Dependency, assetLibrary:AssetLibrary, textureDependency:TextureDependency, options:Options, writerOptions:WriterOptions)
typedef TextureWriterFunction = Dependency -> AssetLibrary -> TextureDependency -> Options -> WriterOptions -> SerializedAsset;
//serializeMaterial(dependency:Dependency, assetLibrary:AssetLibrary, material:Material, resourceId:Int, options:Options, writerOptions:WriterOptions)
typedef MaterialWriterFunction = Dependency -> AssetLibrary -> Material -> Int -> Options -> WriterOptions -> SerializedAsset;
...
class Dependency {
    ...
    public function new() {
        ...
        if (_textureWriteFunction == null) {
            setTextureFunction(Dependency.serializeTexture) ;
        }

        if (_materialWriteFunction == null) {
            setMaterialFunction(Dependency.serializeMaterial);
        }
    }
    ...
    public function serializeGeometry(dependency:Dependency, assetLibrary:AssetLibrary, geometry:Geometry, resourceId:Int, options:Options, writerOptions:WriterOptions, userDefinedDependency:Array<SerializedAsset>) {
        ...
        var content:Bytes = Bytes.ofString("");
        ...
        if (!assetIsNull && writerOptions.embedMode & EmbedMode.Geometry) {
            ...
            if (!assetIsNull) {
                geometryWriter.write(writeFilename, assetLibrary, options, writerOptions, dependency, userDefinedDependency);
            }
            content = Bytes.ofString(outputFilename);
        }
    ...
    public function serializeTexture(dependency:Dependency, assetLibrary:AssetLibrary, textureDependency:TextureDependency, options:Options, writerOptions:WriterOptions) {
        ...
        var res:SerializedAsset = new SerializedAsset(assetType, dependencyId, content);
    ...
    public function serializeEffect(dependency:Dependency, assetLibrary:AssetLibrary, effect:Effect, resourceId:Int, options:Options, writerOptions:WriterOptions) {
        ...
        content = Bytes.ofString(File.removePrefixPathFromFilename(filename));
    ...
    public function serialize(parentFilename:String, assetLibrary:AssetLibrary, options:Options, writerOptions:WriterOptions) {
        ...
        for (linkedAssetToIdPair in _linkedAssetDependencies.keys()) {
            var linkedAsset:LinkedAsset = linkedAssetToIdPair;
            var id = _linkedAssetDependencies.get(linkedAssetToIdPair);
            var res = serializeLinkedAsset(this, assetLibrary, linkedAsset, id, options, writerOptions);
            serializedAsset.push(res);
        }
    ...
    public function serializeLinkedAsset(dependency:Dependency, assetLibrary:AssetLibrary, linkedAsset:LinkedAsset, resourceId:Int, options:Options, writerOptions:WriterOptions) {
        var assetType:AssetType = 0 ;
        var content:Bytes = Bytes.ofString("");
        var serializedLinkedAsset:SerializedAsset;
        switch (linkedAsset.linkType)
        {
            case LinkedAsset.LinkType.Internal:{
                assetType = AssetType.EMBED_LINKED_ASSET;
                content = Bytes.alloc(linkedAsset.length);
                content.blit(0, linkedAsset.data, linkedAsset.offset, linkedAsset.length);
                break;
            }
            case LinkedAsset.LinkType.External:{
                assetType = AssetType.LINKED_ASSET;
                //index 2  of  linkedAsset.offset   linkedAsset.length
                var outBytes:BytesOutput = new BytesOutput();
                outBytes.writeInt32(linkedAsset.offset);
                outBytes.writeInt32(linkedAsset.length);
                outBytes.writeUTF(File.removePrefixPathFromFilename(linkedAsset.filename));
                content = outBytes.getBytes();
                break;
            }
            default:
                break;
