...
class Signal5<A, B, C, D, E> {
    ...
    public function copyFrom(other:Signal4<A, B, C, D, E>) {
    ...
    public function remove(it:SignalSlot5<A, B, C, D, E>) {
        _callbacks = _callbacks.filter(function(it) {
            return it.second != it;
        });
    }
