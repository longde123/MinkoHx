...
class RootTransform extends AbstractComponent {
    ...
    override public function dispose() {
        ...
        _nodeToId = null;
        for(_n in _nodeToPropertyChangedSlot)
            _n.dispose();
    ...
    public function updateTransformsList() {
        ...
        for (toRemove in _toRemove) {
            ...
            _nodeToPropertyChangedSlot.get(toRemove).disconnect();
    ...
    public function updateTransforms() {
        ...
        for (node in _nodes) {
            ...
            if (nodeCacheEntry._dirty) {
                ...
                //todo fix !=
                if (!MathUtil.mat4_equals(nodeCacheEntry._modelToWorldMatrix, modelToWorldMatrix)) {
                    var nodeData:Store = node.data;
                    var provider = nodeCacheEntry._provider;

                    // manually update the data provider internal mat4 object
                    MathUtil.mat4_copyFrom(nodeCacheEntry._modelToWorldMatrix, modelToWorldMatrix);
...
class NodeTransformCacheEntry {
    ...
    public var _matrix:Mat4;
    public var _modelToWorldMatrix:Mat4;
...
class Transform extends AbstractComponent {
    ...
    private var _modelToWorld:Mat4;
    ...
    function set_matrix(value) {
        ...
        if (!MathUtil.mat4_equals(value, _matrix)) {
            MathUtil.mat4_copyFrom(_matrix, value);
        } else {
            return value;
    ...
    override public function targetRemoved(target:Node) {
        ...
        if (_addedSlot != null) {
            _addedSlot.disconnect();
        }
        _addedSlot = null;
        if (_removedSlot != null) {
            _removedSlot.disconnect();
        }
    ...
    public function new() {
        super();
        //	_worldToModel(1.),
        this._matrix = Mat4.identity(new Mat4());
        this._modelToWorld = Mat4.identity(new Mat4());
        this._data = Provider.create();
        this._dirty = false;
        this._data.set("matrix", _matrix)
        .set("modelToWorldMatrix", _modelToWorld);
    }
