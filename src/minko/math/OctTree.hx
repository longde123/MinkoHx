...
class OctTree {
    ...
    public function new(worldSize:Float, maxDepth:Int, center:Vec3, depth:Int) {
        ...
        _octantBox = Box.createbyVector3(MathUtil.Vector3_add(_center, halfEdgeLength), MathUtil.Vector3_sub(_center, halfEdgeLength));
    ...
    public function generateVisual(assetLibrary:AssetLibrary, rootNode:Node = null) {
        ...
        if (_content.length > 0) {
            ...
            var matrix:Mat4 = Mat4.translation(_center.x, _center.y, _center.z).multmat(Mat4.scale(edgeLength, edgeLength, edgeLength));
