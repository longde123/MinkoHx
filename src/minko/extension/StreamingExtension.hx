...
class StreamingExtension extends AbstractExtension {
    ...
    public function serializePOPGeometry(dependency:Dependency, assetLibrary:AssetLibrary, geometry:Geometry, resourceId:Int, options:Options, writerOptions:WriterOptions, includeDependencies:Array<SerializedAsset>) {
        ...
        if (!assetIsNull && writerOptions.embedMode & WriterOptions.EmbedMode.Geometry) {
            ...
            if (!assetIsNull) {
                writer.writeEmbedded(writeFilename, assetLibrary, options, writerOptions, dependency, null, headerData);
    ...
    public function deserializePOPGeometry(metaData:Int, assetLibrary:AssetLibrary, options:Options, completePath:String, data:BytesOutput, dependencies:Dependency, assetRef:Int, jobList:Array<Job>) {
        var hasHeader = false;
    ...
    public function deserializeStreamedTexture(metaData:Int, assetLibrary:AssetLibrary, options:Options, assetCompletePath:String, data:Bytes, dependencies:Dependency, assetRef:Int, jobList:Array<Job>) {
        var hasHeader = false;
        var streamedAssetHeaderSize = 0;
