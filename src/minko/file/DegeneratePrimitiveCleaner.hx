...
class Options {
    ...
    public function copyFrom(v:Options):Void {

    }
}
class DegeneratePrimitiveCleaner extends AbstractWriterPreprocessor<Node> {
    ...
    override public function process(node:T, assetLibrary:AssetLibrary) {
        ...
        for (i in 0...numIndices / primitiveSize) {
            ...
            if (!degeneratePrimitive && _options.useMinPrecision) {
                ...
                for (j in 0... primitiveSize) {
                    ...
                    var position = new Vec3(positionVertexBufferData[p_index], positionVertexBufferData[p_index + 1], positionVertexBufferData[p_index + 2]);
