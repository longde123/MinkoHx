...
typedef GLSLBlock = Tuple<GLSLBlockType, String>;
...
class MacroBlock extends Block< MacroBindingMap> {
    ...
    public function copyFrom(s:MacroBlock) {
        MacroBindingMap.copyFrom(bindingMap, s.bindingMap);
        return this;
    }
}
...
class Scope {
    ...
    public function copyFromParent(scope:Scope, parent:Scope) {
        copyFrom(scope);
        this.parent = parent;
...
class EffectParser extends AbstractParser {
    ...
    public function parseTechniques(node:Any, scope:Scope, techniques:Techniques) {
        ...
        if (Std.is(techniquesNode, Array)) {
            ...
            for (techniqueNode in tmp) {
                ...
                parsePasses(techniqueNode, techniqueScope, techniques.get(techniqueName));
        ...
        for (blockIt in blocks.leaf) {
            ...
            if (block.first == GLSLBlockType.FILE) {
                if (options.assetLibrary.hasBlob(block.second)) {
                    var data:Bytes = options.assetLibrary.blob(block.second);
                ...
                block.second = "//#pragma include(\"" + block.second + "\")\n";
                    ...
                    _loaderCompleteSlots.set(loader, loader.complete.connect(function(_1) {
                        glslIncludeCompleteHandler(_1, blockIt, block.second, includes);
                    }, 0, true)
                    );

                    _loaderErrorSlots.set(loader, loader.error.connect(function(_1, _2) {
                        dependencyErrorHandler(_1, _2, block.second);
                    }, 0, true));
    ...
    public function createStates(block:StateBlock) {
        return null ;
    }
