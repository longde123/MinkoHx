...
class Loader {
    ...
    public function protocolCompleteHandler(protocol:AbstractProtocol) {
        ...
        _protocolErrorSlots.get(protocol).disconnect();
        _protocolErrorSlots.remove(protocol);
        _protocolCompleteSlots.get(protocol).disconnect();
        _protocolCompleteSlots.remove(protocol);
        _protocolProgressSlots.get(protocol).disconnect();
    ...
    public function finalize() {
        if (_loading.length == 0 && _filesQueue.length == 0 && _numFilesToParse == 0) {
