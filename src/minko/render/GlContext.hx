...
class GlContext extends AbstractContext {
    ...
    override public function clear(red = 0.0, green = 0.0, blue = 0.0, alpha = 0.0, depth = 1.0, stencil = 0, mask = 0xffffffff) {
...
//#if DEBUG
        if (_errorsEnabled && getError() != GL.NO_ERROR) {
            trace("error: OpenGLES2Context::checkForErrors()");
            trace("\n");
            throw "";
        }
//#end
    ...
    override public function createVertexBuffer(size) {
        ...
        gl.bufferData(GL.ARRAY_BUFFER, size * 4, GL.STATIC_DRAW);
        vertexBufferCount += 1;
...
*/
    ...
    override public function setVertexAttributeArray(vertexArray) {
    ...
    override public function createIndexBuffer(size) {
        var indexBuffer:Buffer = gl.createBuffer();
        gl.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, indexBuffer);


        gl.bufferData(GL.ELEMENT_ARRAY_BUFFER, size * 2, GL.STATIC_DRAW);
        indexBufferCount += 1;
    ...
    override public function uploaderIndexBufferData(indexBuffer:Int, offset:Int, size:Int, data:Array<Int>) {
        ...
        gl.bufferSubData(GL.ELEMENT_ARRAY_BUFFER, offset * 2, new Uint16Array(data));
        checkForErrors();
    }
    ...
    override public function createTexture(type:TextureType, width, height, mipMapping:Bool, optimizeForRenderToTexture = false, assertPowerOfTwoSized = true) {
    ...
    override public function createRectangleTexture(type:TextureType, width, height) {
        return createTexture(type, width, height, false, false, false);
    }

    override public function createCompressedTexture(type:TextureType, format:TextureFormat, width, height, mipMapping) {
    ...
    public function getTextureType(textureId) {
...
//Debug.Assert(foundTypeIt != _textureTypes.end());
    ...
    override public function uploadTexture2dData(texture, width, height, mipLevel, data:Bytes) {
//Debug.Assert(getTextureType(texture) == TextureType.Texture2D);
    ...
    override public function uploadCubeTextureData(texture, face:CubeTexture.Face, width, height, mipLevel, data:Bytes) {
//Debug.Assert(getTextureType(texture) == TextureType.CubeTexture);
    ...
    override public function uploadCompressedTexture2dData(texture, format:TextureFormat, width, height, size, mipLevel, data:Bytes) {
//Debug.Assert(getTextureType(texture) == TextureType.Texture2D);
    ...
    override public function uploadCompressedCubeTextureData(texture, face:CubeTexture.Face, format:TextureFormat, width, height, mipLevel, data:Bytes) {

        // FIXME
        throw "";
    }

    override public function activateMipMapping(texture) {
        _textureHasMipmaps.set(texture, true);
    }

    override public function deleteTexture(texture) {
    ...
    override public function setTextureAt(position, texture, location = -1) {
    ...
    override public function setSamplerStateAt(position, wrapping:WrapMode, filtering:TextureFilter, mipFiltering:MipFilter) {
    ...
    override public function attachShader(program, shader) {
        gl.attachShader(_programs.get(program), _shaders.get(shader));

        checkForErrors();
    }

    override public function linkProgram(program) {
    ...
    override public function deleteProgram(program) {
    ...
    override public function compileShader(shader) {
    ...
    override public function setProgram(program) {
    ...
    override public function setShaderSource(shader, source) {
    ...
    public function getShaderSource(shader, source) {
        source = gl.getShaderSource(_shaders.get(shader));
        checkForErrors();
    }
    ...
    override public function deleteVertexShader(vertexShader) {
    ...
    override public function deleteFragmentShader(fragmentShader) {
    ...
    override public function getProgramInputs(program) {
        setProgram(program);
    ...
    public function convertInputType(type) {
        switch (type)
        {
            case GL.FLOAT:
    ...
    public function getUniformInput(program, name):ProgramInputs.UniformInput {
    ...
    public function getUniformInputs(program):Array<ProgramInputs.UniformInput> {
    ...
    public function getAttributeInput(program, name):ProgramInputs.AttributeInput {
    ...
    public function getAttributeInputs(program):Array<ProgramInputs.AttributeInput> {
...
//glGetProgramiv(program, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, maxAttributeNameLength);
    ...
    public function getShaderCompilationLogs(shader) {
...
//var buffer:String =gl.getShaderSource(_shaders.get(shader));
    ...
    public function getProgramInfoLogs(program) {
    ...
    override public function setStencilTest(stencilFunc:CompareMode, stencilRef, stencilMask, stencilFailOp:StencilOperation, stencilZFailOp:StencilOperation, stencilZPassOp:StencilOperation) {
    ...
    override public function readRectPixels(x, y, width, height, pixels:Bytes) {
    ...
    function createRTTBuffers(type, textureKey, width, height) {
    ...
    override public function setRenderToTexture(texture, enableDepthAndStencil = false) {
    ...
    override public function generateMipmaps(texture) {
        gl.bindTexture(GL.TEXTURE_2D, _textures.get(texture));
    ...
    public function supportsExtension(extensionNameString) {
