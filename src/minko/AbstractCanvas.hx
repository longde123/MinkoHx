...
class AbstractCanvas {
    ...
    public var desiredFramerate(get, null):Float;
    ...
    public var frameDuration(get, null):Float;
    public var relativeTime(get, null):Float;
    ...
    public function new(name, width, height, flags) {
        ...
        this._startTime = haxe.Timer.stamp();
        this._framerate = 0.0;
        this._desiredFramerate = 60.0;
    ...
    public function step() {
        ...
        var absoluteTime = haxe.Timer.stamp();
        _relativeTime = (absoluteTime - _startTime) ; // in milliseconds
        Timer.update();
        _frameDuration= _deltaTime = Timer.tmod/_desiredFramerate;

        if (_enableRendering) {
            _enterFrame.execute(that, _relativeTime * 1000.0, _deltaTime * 1000.0);
        ...
        _framerate = Timer.fps();

        var remainingTime = (1 / _desiredFramerate) - _frameDuration;
    ...
    public function setDesiredFramerate(desiredFramerate) {
        _desiredFramerate = desiredFramerate;
        Timer.wantedFPS=desiredFramerate;
    }
