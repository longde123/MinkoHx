...
@:expose("minko.geometry.TeapotGeometry")
class TeapotGeometry extends Geometry {

    public static function create(context:AbstractContext, divs = 10) {
        var tp = new TeapotGeometry();
    ...
    static private var _patches:Array<Array<Int>> =
    [
    ...
    static private var _points:Array<Array<Float>> =
    [
        [1.4, 0.0, 2.4],
        [1.4, -0.784, 2.4],
        [0.784, -1.4, 2.4],
    ...
    private function initialize(context:AbstractContext, divs) {
        var indicesData:Array<Int> = [];
        var vertices:Array<Array<Float>> = [[]];
        var patch:Array<Array<Float>> = [for (i in 0... 16)[]];

        var currentVertexId = 0;

        for (bezierPatch in _patches) {
            for (i in 0... 16) {
        ...
        removeDuplicateVertices(indicesData, vertices, Math.floor(vertices[0].length / 3));

        var vb:VertexBuffer = VertexBuffer.createbyData(context, vertices[0]);

        vb.addAttribute("position", 3);
        addVertexBuffer(vb);

        indices = (IndexBuffer.createbyData(context, indicesData));

        computeCenterPosition();
    }

    private function genPatchVertices(patch:Array<Array<Float>>, divs:Int, vertices:Array<Float>) {
        var zero3:Array<Float> = [ for (i in 0...3) 0.0];
        var last:Array<Array<Float>> = [ for (i in 0...divs + 1) zero3.copy()] ;
        var tmp:Array<Array<Float>> = [ for (i in 0...4) zero3.copy()] ;
    ...
    private function genPatchIndices(offset, divs, indices:Array<Int>) {
        for (u in 0...divs) {
            for (v in 0... divs) {
    ...
    private function bernstein(u:Float, p0:Array<Float>, p1:Array<Float>, p2:Array<Float>, p3:Array<Float>, _out:Array<Float>) {
        var um = 1.0 - u;
        var um2 = um * um;
        var u2 = u * u;
